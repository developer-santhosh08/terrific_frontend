    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            ...
            try {
                ...
                const poPromise = id ? fetch(`${import.meta.env.VITE_API_BASE_URL}/api/inventory/purchase-orders/${id}`, { headers }).then(r => r.json()) : Promise.resolve(null);

                const [bankJson, poJson] = await Promise.all([bankPromise, poPromise]);

                if (bankJson && bankJson.status && bankJson.data) {
                    setBankOptions(bankJson.data.map(b => ({ value: b.id, label: b.name })));
                }

                if (poJson && poJson.status && poJson.data) {
                    const d = poJson.data;
                    
                    let totalDis = 0;
                    const products = (d.details || []).map((det, i) => {
                    ...
                    setPoData({
                        vendorName: d.vendor ? d.vendor.name : '',
                        receiptNumber: 'New',
                        receiptDate: new Date().toISOString().split('T')[0],
                        products: products,
                        totalDiscount: totalDis,
                        totalAmount: d.net_value,
                        balanceAmount: d.balance_amount
                    });
                    
                    setForm(prev => ({ ...prev, amount: d.balance_amount?.toString() || '' }));
                }
            } catch (error) {
            ...
            <div className="container-fluid">
                <div className="card">
                    ...
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title">Edit Advanced Payment {id ? `#${id}` : ''}</h3>
                        <button
                            type="button"
                    ...
                    <div className="card-body">
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="row mt-3">
                                ...
                                {form.paymentMode && form.paymentMode.label.toLowerCase() !== 'cash' && (
                                    <>
                                        <div className="col-md-3 form-group">
                                            <label>Bank</label>
                                            <Select
