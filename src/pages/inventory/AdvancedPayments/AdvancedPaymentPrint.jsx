...
const styles = StyleSheet.create({
    ...
    colPrice: { width: '13%', borderRightWidth: 1, borderRightColor: '#000', padding: 5, textAlign: 'right', justifyContent: 'center' },
    ...
    colTotal: { width: '14%', padding: 5, textAlign: 'right', justifyContent: 'center' },
        ...
        <Document title={header?.vendor_name ? `Receipt-${header?.po_no ? `${header.po_no} ` : ''}${header.vendor_name}` : 'Advanced Payment Receipt'}>
            <Page size="A4" style={styles.page}>
                <View style={styles.pageBorder}>
                    ...
                    <Text style={styles.title}>Advance Payment Receipt</Text>

                    <View style={styles.companyHeader}>
                        <View style={styles.logoTextContainer}>
                            <Text style={styles.logoText}>Terrific Technologies</Text>
                    ...
                    <View style={{ flex: 1, flexDirection: 'row' }}>
                        <View style={{ width: '65%', ...styles.col, borderBottomWidth: 0, padding: 10 }}>
                            ...
                            {(header?.terms_of_delivery || header?.terms_of_payment) && (
                                <View style={{ marginTop: 10 }}>
                                    <Text style={{ ...styles.bold, textDecoration: 'underline', marginBottom: 5 }}>Terms & Conditions</Text>
                                    {header?.terms_of_delivery ? <View style={styles.flexRowBetween}><Text style={{ width: '40%', ...styles.bold }}>Delivery:</Text><Text style={{ width: '60%' }}>{header.terms_of_delivery}</Text></View> : null}
                                    {header?.terms_of_payment ? <View style={styles.flexRowBetween}><Text style={{ width: '40%', ...styles.bold }}>PAYMENT:</Text><Text style={{ width: '60%' }}>{header.terms_of_payment}</Text></View> : null}
                                </View>
                            )}
                        </View>
...
const AdvancedPaymentPrint = () => {
    ...
    useEffect(() => {
        const fetchPrintData = async () => {
            ...
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/inventory/advance-payments/receipts-print/${id}`, { headers });
    ...
    const fileName = printData.header?.vendor_name ? `Receipt-${printData.header?.po_no ? `${printData.header.po_no} ` : ''}${printData.header.vendor_name}.pdf` : 'Advanced_Payment_Receipt.pdf';
...
export default AdvancedPaymentPrint;
