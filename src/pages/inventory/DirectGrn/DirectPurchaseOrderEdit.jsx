    ...
    useEffect(() => {
        const fetchDropdowns = async () => {
            try {
                ...
                // Products are fetched via useEffect when vendor changes
                // Fetch Taxes
        ...
        if (id) {
            const fetchGrn = async () => {
                ...
                try {
                    ...
                    const listRes = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/inventory/direct-grn`, { headers });
                    const listJson = await listRes.json();
                    const listItem = (listJson.data || []).find(item => Number(item.id) === Number(id)) || {};

                    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/inventory/direct-grn/${id}`, { headers });
                    const result = await res.json();
                    if (result.status && result.data) {
                        const data = result.data;
                        const header = data.header || data;

                        const stockLocationId = header.stock_location_type_id || header.stock_location_id || header.stock_location || listItem.stock_location_type_id;
                        const vendorId = header.vendor_id || listItem.vendor_id;

                        setForm(p => ({
                            ...p,
                            grnNumber: header.grn_number || header.po_no || header.purchase_order_number || data.id || p.grnNumber,
                            stockLocation: stockLocationId ? { value: stockLocationId, label: header.stock_location_type?.type || header.stock_location || 'Stock Location' } : p.stockLocation,
                            vendorName: vendorId ? { value: vendorId, label: header.vendor?.name || header.vendor_name || 'Vendor' } : p.vendorName,
                            date: header.purchase_order_date ? header.purchase_order_date.split(' ')[0] : (header.po_date ? header.po_date.split(' ')[0] : (header.date || p.date))
                        }));

                        const items = data.details || data.products || [];
                        if (items && items.length > 0) {
                            setRows(items.map((prod, index) => {
                                let matchedTax = '';
                                if (prod.tax_percent !== undefined) {
                                    if (Number(prod.tax_percent) === 0) matchedTax = 'NO_TAX_0';
                                    else if (Number(prod.tax_percent) === 18) matchedTax = 'GST_18';
                                    else if (Number(prod.tax_percent) === 9) matchedTax = 'CGST_9';
                                }
                                return {
                                    id: prod.id || Date.now() + index,
                                    productName: prod.vendor_product_mapping_id || prod.product_id || prod.product_name || '',
                                    qty: prod.quantity || 0,
                                    price: prod.unit_price || prod.price || prod.rate || 0,
                                    tax: prod.tax_id || prod.tax || '',
                                    discount: prod.scheme_disc || prod.discount || 0
                                };
                            }));
                        }
                        
                        setRoundOff(header.round_off !== undefined && header.round_off !== null ? header.round_off : '');
                    }
                } catch (e) {
                    console.error(e);
                } finally {
        ...
        try {
            ...
            const payload = {
                ...
                products: rows.map(r => {
                    const pId = (productOptions.find(o => String(o.value) === String(r.productName) || String(o.label) === String(r.productName)) || {}).value || r.productName;
                    return {
                        id: r.id && String(r.id).length < 10 ? r.id : null,
                        vendor_product_mapping_id: pId,
                        product_id: pId,
                        quantity: Number(r.qty) || 0,
                        price: Number(r.price) || 0,
            ...
            const url = `${import.meta.env.VITE_API_BASE_URL}/api/inventory/direct-grn/${id}`;
            ...
            <div className="container-fluid">
                <div className="card">
                    ...
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title">Edit Direct GRN</h3>
                            ...
                            <div className="row">
                                ...
                                <div className="col-md-3 form-group">
                                    ...
                                    <Select
                                        ...
                                        value={stockLocationOptions.find(o => Number(o.value) === Number(form.stockLocation?.value)) || form.stockLocation}
                                ...
                                <div className="col-md-3 form-group">
                                    ...
                                    <Select
                                        ...
                                        value={vendorOptions.find(o => Number(o.value) === Number(form.vendorName?.value) || String(o.label) === String(form.vendorName?.label)) || form.vendorName}
                                            ...
                                            <tr key={row.id}>
                                                <td>
                                                    <Select
                                                        ...
                                                        value={productOptions.find(o => String(o.value) === String(row.productName) || String(o.label) === String(row.productName)) || (row.productName ? { label: row.productName, value: row.productName } : null)}
                                                        onChange={opt => {
                                                ...
                                                <td>
                                                    <Select
                                                        options={taxOptions}
                                                        value={taxOptions.find(o => String(o.value) === String(row.tax) || Number(o.rate) === Number(row.tax)) || null}
...
export default DirectPurchaseOrderEdit;
