...
const PODocument = ({ data }) => {
    const { header, details, company, special_instructions } = data;
    
    let subTotal = 0;
    let taxTotal = 0;
    const safeDetails = details || [];
    const calculatedDetails = safeDetails.map(item => {
        const qty = Number(item.quantity) || 0;
        const price = Number(item.unit_price) || 0;
        const disc = Number(item.scheme_disc) || 0;
        ...
        return {
            ...item,
            calculated_tax_amount: taxAmt,
            calculated_total: base + taxAmt
        };
    });
    ...
    return (
        <Document title={header?.vendor_name ? `${header?.po_no ? `PO-${header.po_no} ` : ''}${header.vendor_name}` : 'Purchase Order'}>
            <Page size="A4" style={styles.page}>
                <View style={styles.pageBorder}>
                    
                    <Text style={styles.title}>Direct GRN</Text>
                    ...
                    <View style={styles.row}>
                        ...
                        <View style={styles.colMeta}>
                            <View style={styles.flexRowBetween}>
                                <Text>GRN No</Text><Text>: {header?.po_no || header?.grn_number}</Text>
                            </View>
                            <View style={styles.flexRowBetween}>
                                <Text>Date</Text><Text>: {header?.po_date ? header?.po_date?.split(' ')[0] : (header?.purchase_order_date || '').split(' ')[0]}</Text>
                            </View>
                            <View style={styles.flexRowBetween}>
                    ...
                    {calculatedDetails.map((item, i) => (
                        <View style={styles.itemsRow} key={i}>
                            ...
                            <View style={{...styles.colTax, alignItems: 'center'}}>
                                <Text>{item.tax_percent || 0}%</Text>
                                <Text>{Number(item.calculated_tax_amount).toFixed(2)}</Text>
                            </View>
                            <View style={styles.colTotal}><Text>{Number(item.calculated_total).toFixed(2)}</Text></View>
                        </View>
                    ))}
                    ...
                    <View style={styles.row}>
                        ...
                        <View style={styles.summaryRight}>
                            ...
                            <View style={styles.summaryValueCol}>
                                <Text style={styles.bold}>{taxTotal.toFixed(2)}</Text>
                                <Text style={styles.bold}>{grandTotal.toFixed(2)}</Text>
                            </View>
                        </View>
                    </View>
                    ...
                    <View style={styles.row}>
                        ...
                        <View style={{ width: '25%', padding: 5, alignItems: 'flex-end' }}>
                            <Text style={{...styles.bold, color: '#dc2626'}}>{grandTotal.toFixed(2)}</Text>
                        </View>
                    </View>
...
const DirectPurchaseOrderPrint = () => {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchPrintData = async () => {
            ...
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/inventory/direct-grn/${id}`, { headers });
    ...
    const fileName = printData.header?.vendor_name ? `${printData.header?.po_no ? `GRN-${printData.header.po_no} ` : ''}${printData.header.vendor_name}.pdf` : 'Direct_GRN.pdf';
...
export default DirectPurchaseOrderPrint;
