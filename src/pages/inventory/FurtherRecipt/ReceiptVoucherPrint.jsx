...
const VoucherDocument = ({ data, type, isCancelled }) => {
    ...
    const address = [header?.vendor_address1, header?.vendor_address2, header?.vendor_address3].filter(Boolean).join(', ');

    return (
        <Document title={header?.vendor_name ? `Receipt-${receipt_number}-${header.vendor_name}` : 'Receipt'}>
            <Page size="A4" style={styles.page}>
                ...
                <View style={[styles.container, { padding: 0 }]}>
                    ...
                    <View style={styles.contentRow}>
                        ...
                        <Text style={styles.valueText}>: {header?.vendor_name} ({header?.vendor_mobile})</Text>
                    </View>
...
const ReceiptVoucherPrint = () => {
    ...
    const apiEndpoint = isAdvance 
        ? `${import.meta.env.VITE_API_BASE_URL}/api/inventory/advance-payments/receipt-single-print/${id}`
        : `${import.meta.env.VITE_API_BASE_URL}/api/inventory/further-payments/receipt-single-print/${id}`;
