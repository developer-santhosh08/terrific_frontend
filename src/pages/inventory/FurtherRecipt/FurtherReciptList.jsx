...
const FurtherReciptList = () => {
    const { refreshPermissions, hasPermission } = usePermissions();
    const { setLoading } = useLoader();
    const navigate = useNavigate();

    const canViewEnquiry = hasPermission('Inventory.Further Receipt List - Enquiry List in Receipt.View');
    const canViewReceiptCompletedEnquiry = hasPermission('Inventory.Further Receipt List - Receipt Completed Enquiry List.View');
    const canViewCollectionPending = hasPermission('Inventory.Further Receipt List - Collection Pending List.View');
    const canViewReceipt = hasPermission('Inventory.Further Receipt List - Receipt List.View');
    const canViewCancelledReceipt = hasPermission('Inventory.Further Receipt List - Cancelled Receipt List.View');
    ...
    const activeTabLabel = 
        activeTab === 'enquiry' ? 'Further Receipt List - Enquiry List in Receipt' :
        activeTab === 'receipt_completed_enquiry' ? 'Further Receipt List - Receipt Completed Enquiry List' :
        activeTab === 'collection_pending' ? 'Further Receipt List - Collection Pending List' :
        activeTab === 'receipt' ? 'Further Receipt List - Receipt List' :
        'Further Receipt List - Cancelled Receipt List';

    const canEdit = hasPermission(`Inventory.${activeTabLabel}.Edit`);
    const canPrint = hasPermission(`Inventory.${activeTabLabel}.Print`);
    const canApprove = hasPermission(`Inventory.${activeTabLabel}.Approve`);
    const canAdd = hasPermission(`Inventory.${activeTabLabel}.Add`);

    const hasActionPermission = (() => {
        if (activeTab === 'enquiry') return canEdit || canPrint;
        if (activeTab === 'receipt_completed_enquiry') return canPrint;
        if (activeTab === 'collection_pending') return canEdit || canPrint || canApprove || canAdd;
        if (activeTab === 'receipt') return canPrint;
        if (activeTab === 'cancelled_receipt') return canPrint;
        return false;
    })();

    useEffect(() => {
        if (!canViewEnquiry && activeTab === 'enquiry') {
            if (canViewReceiptCompletedEnquiry) setActiveTab('receipt_completed_enquiry');
            else if (canViewCollectionPending) setActiveTab('collection_pending');
            else if (canViewReceipt) setActiveTab('receipt');
            else if (canViewCancelledReceipt) setActiveTab('cancelled_receipt');
        }
        if (!canViewReceiptCompletedEnquiry && activeTab === 'receipt_completed_enquiry') {
            if (canViewEnquiry) setActiveTab('enquiry');
            else if (canViewCollectionPending) setActiveTab('collection_pending');
            else if (canViewReceipt) setActiveTab('receipt');
            else if (canViewCancelledReceipt) setActiveTab('cancelled_receipt');
        }
        if (!canViewCollectionPending && activeTab === 'collection_pending') {
            if (canViewEnquiry) setActiveTab('enquiry');
            else if (canViewReceiptCompletedEnquiry) setActiveTab('receipt_completed_enquiry');
            else if (canViewReceipt) setActiveTab('receipt');
            else if (canViewCancelledReceipt) setActiveTab('cancelled_receipt');
        }
        if (!canViewReceipt && activeTab === 'receipt') {
            if (canViewEnquiry) setActiveTab('enquiry');
            else if (canViewReceiptCompletedEnquiry) setActiveTab('receipt_completed_enquiry');
            else if (canViewCollectionPending) setActiveTab('collection_pending');
            else if (canViewCancelledReceipt) setActiveTab('cancelled_receipt');
        }
        if (!canViewCancelledReceipt && activeTab === 'cancelled_receipt') {
            if (canViewEnquiry) setActiveTab('enquiry');
            else if (canViewReceiptCompletedEnquiry) setActiveTab('receipt_completed_enquiry');
            else if (canViewCollectionPending) setActiveTab('collection_pending');
            else if (canViewReceipt) setActiveTab('receipt');
    ...
    const handleCollectSubmit = async (bankId) => {
        ...
        try {
            const token = sessionStorage.getItem('token');
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/inventory/further-payments/receipts/${selectedReceiptId}/collect`, {
                method: 'PUT',
                headers: {
        ...
        if (tabKey === 'enquiry') {
            return (
                <div className="tw-flex tw-gap-2 tw-items-center">
                    ...
                    {canPrint && (
                        <button type="button" className="list-action-btn btn-print" title="Print" onClick={() => window.open(`/print/further-receipt/${row.id}`, '_blank')}>
        ...
        if (tabKey === 'collection_pending' || tabKey === 'receipt' || tabKey === 'cancelled_receipt') {
            return (
                <div className="tw-flex tw-gap-2 tw-items-center">
                    ...
                    {canPrint && (
                        <button type="button" className="list-action-btn btn-print" title="Print" onClick={() => window.open(`/print/further-receipt-voucher/${row.id}${tabKey === 'cancelled_receipt' ? '?cancelled=true' : ''}`, '_blank')}>
                            <Printer weight="bold" className="tw-w-4 tw-h-4" />
                        </button>
                    )}
        ...
        if (tabKey === 'enquiry') {
            return (
                ...
                <div className="md:tw-hidden tw-flex tw-flex-col tw-mt-4">
                        ...
                        <MobileCard key={r.id}>
                            ...
                            <MobileCard.Body>
                                <div className="tw-grid tw-grid-cols-2 tw-gap-2 tw-mb-2">
                                    <MobileCard.Field label="PO Date" value={r.poDate} />
                                    <MobileCard.Field label="Product Type" value={r.productType} align="right" />
                                    <MobileCard.Field label="Amount Details" value={
                                        <div className="tw-flex tw-flex-col">
                                            <span className="tw-text-sm">Balance: <strong style={{ color: '#ef4444' }}>{r.balance}</strong></span>
                                            <span className="tw-text-sm">Paid: <strong style={{ color: '#16a34a' }}>{r.paid}</strong></span>
                                        </div>
                                    } />
                                    <MobileCard.Field label="Vendor Name" value={r.customerName} align="right" />
                                    <MobileCard.Field label="Mobile" value={r.mobile} />
                                    <MobileCard.Field label="Invoice Details" value={<Badge value={r.invoiceDetails} map={BADGE_STYLE} />} align="right" />
                                    <MobileCard.Field label="Stage" value={<Badge value={r.currentStage} map={BADGE_STYLE} />} />
                                </div>
                            </MobileCard.Body>
                            {hasActionPermission && (
                                <MobileCard.Footer>
                    ...
                    {items.length === 0 && (
                        <div className="tw-bg-white tw-rounded-lg tw-border tw-border-slate-200 tw-p-8 tw-text-center tw-text-slate-500 tw-mb-4">No data available in table</div>
                    )}
                </div>
                </>
            );
                ...
                <div className="md:tw-hidden tw-flex tw-flex-col tw-mt-4">
                    {items.map((r, idx) => (
                        <MobileCard key={r.id}>
                            ...
                            <MobileCard.Body>
                                <div className="tw-grid tw-grid-cols-2 tw-gap-2 tw-mb-2">
                                    <MobileCard.Field label="Vendor Name" value={r.customerName} />
                                    ...
                                    <MobileCard.Field label="Stage" value={<Badge value={r.currentStage} map={BADGE_STYLE} />} align="right" />
                                </div>
                            </MobileCard.Body>
                            {hasActionPermission && (
                                <MobileCard.Footer>
            ...
            return (
                ...
                <div className="table-responsive">
                    <table className="table table-bordered table-striped no-margin">
                        ...
                        <tbody>
                            ...
                            {items.length === 0 && (
                                <tr>
                                    <td colSpan={hasActionPermission ? 12 : 11} className="tw-text-center tw-text-slate-400 tw-py-8">No data available in table</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                ...
                <div className="md:tw-hidden tw-flex tw-flex-col tw-mt-4">
                    {items.map((r, idx) => (
                        <MobileCard key={r.id}>
                            <MobileCard.Header label={(tabKey === 'receipt' || tabKey === 'cancelled') ? 'PO. NO' : 'ENQ. NO'} value={r.enqNo} />
                            <MobileCard.Body>
                                <div className="tw-grid tw-grid-cols-2 tw-gap-2 tw-mb-2">
                                    ...
                                    <MobileCard.Field label="Amount" value={r.receiptAmount} align="right" bold />
                                    
                                    {tabKey === 'collection_pending' && (
                                        <>
                                            <MobileCard.Field label="Mobile" value={r.mobile} />
                                            <MobileCard.Field label="Employee" value={r.employeeName} align="right" />
                                        </>
                                    )}
                                    {(tabKey === 'receipt' || tabKey === 'cancelled') && (
                                        <MobileCard.Field label="Mobile" value={r.mobile} />
                                    ...
                                    {tabKey !== 'collection_pending' && tabKey !== 'receipt' && tabKey !== 'cancelled' && (
                                        <>
                                            <MobileCard.Field label="Followup Date" value={
                                                <input type="text" className="form-control form-control-sm" defaultValue={r.followupDate} style={{ minWidth: '100px' }} />
                                            } />
                                            <MobileCard.Field label="Followup Status" value={<Badge value={r.followupStatus} map={BADGE_STYLE} />} align="right" />
                                        </>
                                    )}
                                    <MobileCard.Field label="Status" value={<Badge value={r.receiptStatus} map={BADGE_STYLE} />} />
                                </div>
                            </MobileCard.Body>
                            {hasActionPermission && (
                                <MobileCard.Footer>
            ...
            return (
                ...
                <div className="md:tw-hidden tw-flex tw-flex-col tw-mt-4">
                    {items.map((r, idx) => (
                        <MobileCard key={r.id}>
                            <MobileCard.Header label="PO. NO" value={r.enqNo} />
                            <MobileCard.Body>
                                <div className="tw-grid tw-grid-cols-2 tw-gap-2 tw-mb-2">
                                    <MobileCard.Field label="Receipt Date" value={r.receiptDate} />
                                    <MobileCard.Field label="Receipt No" value={r.receiptNumber} align="right" />
                                    ...
                                    <MobileCard.Field label="Amount" value={r.receiptAmount} align="right" bold />
                                    <MobileCard.Field label="Mobile" value={r.mobile} />
                                    <MobileCard.Field label="Status" value={<Badge value={r.receiptStatus} map={BADGE_STYLE} />} align="right" />
                                </div>
                            </MobileCard.Body>
                            {hasActionPermission && (
                                <MobileCard.Footer>
        ...
        <section className="content">
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border tw-flex tw-flex-col sm:tw-flex-row tw-justify-between tw-items-start sm:tw-items-center tw-gap-4">
                        <h3 className="card-title tw-m-0">Further Receipt</h3>
                    </div>
                    ...
                    <div className="card-body">
                        ...
                        <div className="tw-flex tw-flex-wrap tw-gap-2 tw-mb-3">
                            {canViewEnquiry && (
                                <button
                                    type="button"
                                    className={`tw-px-4 tw-py-2 tw-rounded-sm tw-flex-1 sm:tw-flex-none tw-min-w-[140px] tw-text-center tw-font-medium ${activeTab === 'enquiry' ? 'tw-bg-blue-600 tw-text-white' : 'tw-bg-white tw-border tw-text-gray-700'}`}
                            ...
                            {canViewReceiptCompletedEnquiry && (
                                <button
                                    type="button"
                                    className={`tw-px-4 tw-py-2 tw-rounded-sm tw-flex-1 sm:tw-flex-none tw-min-w-[140px] tw-text-center tw-font-medium ${activeTab === 'receipt_completed_enquiry' ? 'tw-bg-blue-600 tw-text-white' : 'tw-bg-white tw-border tw-text-gray-700'}`}
                            ...
                            {canViewCollectionPending && (
                                <button
                                    type="button"
                                    className={`tw-px-4 tw-py-2 tw-rounded-sm tw-flex-1 sm:tw-flex-none tw-min-w-[140px] tw-text-center tw-font-medium ${activeTab === 'collection_pending' ? 'tw-bg-blue-600 tw-text-white' : 'tw-bg-white tw-border tw-text-gray-700'}`}
                            ...
                            {canViewReceipt && (
                                <button
                                    type="button"
                                    className={`tw-px-4 tw-py-2 tw-rounded-sm tw-flex-1 sm:tw-flex-none tw-min-w-[140px] tw-text-center tw-font-medium ${activeTab === 'receipt' ? 'tw-bg-blue-600 tw-text-white' : 'tw-bg-white tw-border tw-text-gray-700'}`}
                            ...
                            {canViewCancelledReceipt && (
                                <button
                                    type="button"
                                    className={`tw-px-4 tw-py-2 tw-rounded-sm tw-flex-1 sm:tw-flex-none tw-min-w-[140px] tw-text-center tw-font-medium ${activeTab === 'cancelled_receipt' ? 'tw-bg-blue-600 tw-text-white' : 'tw-bg-white tw-border tw-text-gray-700'}`}
            ...
            {isCollectAmountPopupOpen && (
                <CollectAmountPopup 
                    ...
                    onClose={() => { setIsCollectAmountPopupOpen(false); setSelectedReceiptId(null); }} 
