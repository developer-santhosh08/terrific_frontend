...
const PODocument = ({ data }) => {
    const { header, details, company, tax_breakdown, special_instructions } = data;
...
const PurchaseOrderPrint = () => {
    ...
    useEffect(() => {
        const fetchPrintData = async () => {
            ...
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/inventory/purchase-orders/print/${id}`, { headers });
    ...
    const fileName = printData.header?.vendor_name ? `${printData.header?.po_no ? `PO-${printData.header.po_no} ` : ''}${printData.header.vendor_name}.pdf` : 'Purchase_Order.pdf';
