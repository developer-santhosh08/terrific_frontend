...
const PurchaseOrderAdd = () => {
    ...
    useEffect(() => {
        ...
        const fetchPoNumber = async () => {
            try {
                ...
                const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/inventory/purchase-orders/generate-po-number`, { headers });
                const json = await res.json();
                if (json.status && json.data && json.data.po_number) {
                    setForm(prev => ({ ...prev, poNumber: json.data.po_number }));
                }
            } catch (err) {
        ...
        const initData = async () => {
            setLoading(true);
            await Promise.all([fetchVendors(), fetchPoNumber()]);
            setLoading(false);
        };
    ...
    const submitData = async () => {
        ...
        try {
            ...
            const url = `${import.meta.env.VITE_API_BASE_URL}/api/inventory/purchase-orders`;
            const method = 'POST';
            ...
            if (result.status) {
                ...
                window.alert(result.message || 'Failed to save Purchase Order');
            }
        } catch (error) {
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title">Add Purchase Order</h3>
                        <button type="button" className="btn-header-back" onClick={() => navigate('/inventory/purchase-order')}>
                            Back
                        </button>
                    </div>
                                                    ...
                                                    return (
                                                        <tr key={row.id}>
                                                                ...
                                                                <Select
                                                                    ...
                                                                    onChange={(option) => {
                                                                        setRows((prev) => prev.map((r) => 
                                                                            r.id === row.id 
                                                                                ? { ...r, productName: option, price: option?.price || '', discount: option?.discount || '' } 
                                                                                : r
                                                                        ));
                                                                    }}
                                                                    className="react-select-container"
                                                            ...
                                                            <td>
                                                                <Select
                                                                    options={taxOptions}
                                                                    value={taxOptions.find((opt) => opt.id === row.taxId) || taxOptions.find((opt) => Number(opt.percentage) === Number(row.tax))}
                                                                    onChange={(option) => {
                                                                        updateRow(row.id, 'tax', option ? option.percentage : 0);
                                                                        updateRow(row.id, 'taxId', option ? option.id : null);
                                                                    }}
                                                                    className="react-select-container"
                                                                    classNamePrefix="react-select"
                                                                    menuPosition="fixed"
                            ...
                            <div className="row mt-4">
                                <div className="col-12">
                                    <div className="d-flex justify-content-between align-items-center gap-2 flex-wrap pt-3 border-top">
                                        <button type="button" className="btn-cancel" onClick={() => navigate('/inventory/purchase-order')}>
                                            Cancel
                                        </button>
