...
const VendorPaymentPending = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/report/pending-vendor-payment`);
                ...
                console.error("Failed to fetch vendor pending payment data:", error);
            }
            setLoading(false);
        };
    ...
    const searchFilteredData = filteredData.filter(row => {
        ...
        return (
            ...
            String(row.total_pending_amount || '').toLowerCase().includes(searchLower)
        );
    });
    ...
    const exportColumns = [
        ...
        { header: 'Total Pending Amount', key: 'totalPendingAmount' }
    ];
    ...
    const exportData = searchFilteredData.map(row => ({
        ...
        totalPendingAmount: (Number(row.total_pending_amount) || 0).toFixed(2)
    }));
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border">
                        <h3 className="card-title">Pending Vendor Payment Report</h3>
                    </div>
                    <div className="card-body">
                        ...
                        <div className="d-flex flex-column flex-md-row justify-content-md-between align-items-start align-items-md-center tw-gap-2 tw-mb-4 tw-text-sm tw-text-slate-600">
                            <div className="d-flex flex-wrap align-items-center tw-gap-4">
                                ...
                                <ExportButtons 
                                    ...
                                    filename="Vendor_Payment_Pending"
                                    title="Pending Vendor Payment Report"
                                    tableId="vendor-payment-pending-table"
                                />
                            </div>
                            <div className="d-flex align-items-center tw-gap-2">
                                <span>Search:</span>
                        ...
                        <div className="table-responsive tw-hidden md:tw-block">
                            <table id="vendor-payment-pending-table" className="table table-bordered table-striped no-margin">
                                <thead>
                                    <tr>
                                        ...
                                        <th className="tw-align-middle tw-cursor-pointer hover:tw-bg-slate-50" onClick={() => requestSort('total_pending_amount')}>
                                            <div className="tw-flex tw-justify-between tw-items-center">Total Pending Amount <TableSortIcon direction={getSortDirection('total_pending_amount')} /></div>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {loading ? (
                                        ...
                                        paginatedData.map((row, idx) => {
                                            const amount = Number(row.total_pending_amount) || 0;
                        ...
                        <div className="tw-block md:tw-hidden tw-mt-4">
                            {loading ? (
                                ...
                                paginatedData.map((row, idx) => {
                                    ...
                                    return (
                                        <MobileCard key={row.vendor_id || idx}>
                                            <MobileCard.Header label="Sno" value={startIndex + idx + 1} />
                                            <MobileCard.Body>
                                                <div className="tw-grid tw-grid-cols-2 tw-gap-3">
                                                    <MobileCard.Field label="Vendor Name" value={row.vendor_name || '-'} />
                                                    <MobileCard.Field label="Mobile Number" value={row.mobile_number || '-'} />
                                                    <MobileCard.Field label="Total Pending Amount" value={amount.toFixed(2)} />
                                                </div>
                                            </MobileCard.Body>
                                        </MobileCard>
                                    );
...
export default VendorPaymentPending;
