...
const PaymentVoucherEdit = () => {
    ...
    useEffect(() => {
        ...
        const fetchVoucher = async () => {
            ...
            try {
                const token = sessionStorage.getItem('erp_token');
                const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/bankprocess/payment-voucher/${id}`, {
                    headers: {
                        'Accept': 'application/json',
                        ...(token ? { 'Authorization': `Bearer ${token}` } : {})
                    }
                ...
                if ((json.status === 'success' || json.status === true) && json.data) {
                    const d = json.data;
                    setDate(d.document_date || new Date().toISOString().split('T')[0]);
                    setAmount(d.amount || '');
                    setName(d.name || '');
                    setRemark(d.remark || '');
                    
                    // Init salary fields
                    setSalaryAmount(d.salary_amount || '');
                    setSalaryAdvance(d.salary_advance || '');
                    setWorkingDays(d.working_days || '');
                    setLeaveDays(d.leave_days || '');
                    setOtDays(d.ot_days || '');
                    setAlreadyDeductedLeaves(d.already_deducted_leaves || '');
                    setIncentiveAmount(d.incentive_amount || '');
                    setPetrolAmount(d.petrol_amount || '');
                    
                    if (d.payment_voucher_type) {
                        const typeOption = { value: d.payment_voucher_type, label: d.paymentVoucherTypeName || `Type ${d.payment_voucher_type}` };
                        setVoucherType(typeOption);
                        if (typeOption.label.toLowerCase().includes('advance')) {
                            if (d.advance_to === 1) setAdvanceTo(advanceToOptions[0]);
                            else if (d.advance_to === 2) setAdvanceTo(advanceToOptions[1]);
                            else if (d.vendor_id) setAdvanceTo(advanceToOptions[0]);
                            else if (d.employee_id) setAdvanceTo(advanceToOptions[1]);
                        }
                    }
                    if (d.employee_id) setEmployee({ value: d.employee_id, label: d.employee_name || `Emp ${d.employee_id}` });
                    if (d.vendor_id) setVendor({ value: d.vendor_id, label: d.vendor_name || `Vendor ${d.vendor_id}` });
                    if (d.supplier_id) setSupplier({ value: d.supplier_id, label: d.supplier_name || `Supplier ${d.supplier_id}` });
                    if (d.customer_id) setCustomer({ value: d.customer_id, label: d.customer_name || `Customer ${d.customer_id}` });
                    if (d.payment_mode) setPaymentMode({ value: d.payment_mode, label: d.paymentMode === 1 ? 'CASH' : `Mode ${d.payment_mode}` });
                    if (d.deposit_bank_id) setDepositWithdrawAccount({ value: d.deposit_bank_id, label: d.deposit_bank_name || `Account ${d.deposit_bank_id}` });
                    if (d.transfer_type) setTransferType({ value: d.transfer_type === 1 || d.transfer_type === 'withdraw' ? 'withdraw' : 'deposit', label: d.transfer_type === 1 || d.transfer_type === 'withdraw' ? 'Withdraw' : 'Deposit' });
                    if (d.budget_id) setBudgetName({ value: d.budget_id, label: d.budget_name || `Budget ${d.budget_id}` });
                    
                    setTransactionType(d.transaction_type === 1 ? 'Debit' : 'Credit');
                    
                    if (d.suspense_status === 1) {
                        setSuspense(yesNo[0]);
                    } else {
                        setSuspense(yesNo[1]);
                    }
                    
                    if (d.jobcard_status === 1) {
                        setJobcardChecked(true);
                        setIfJobcard(yesNo[0]);
                        if (d.project_id) setSelectedJobcard({ value: d.project_id, label: d.project_id });
                    }
                }
            } catch (err) {
    ...
    const handleConfirmSubmit = async () => {
        ...
        try {
            ...
            if (json.status === 'success' || json.status === true) {
                ...
                console.error('Failed to update payment voucher:', json);
                alert(json.message || 'Failed to update payment voucher');
            }
        } catch (err) {
            console.error('Error updating payment voucher:', err);
            alert('An error occurred while updating payment voucher.');
        } finally {
                    ...
                    <div className="card-body">
                        <form onSubmit={handleFormSubmit}>
                            ...
                            <div className="form-actions mt-3 d-flex justify-content-between">
                                ...
                                <button type="submit" className="btn-save" disabled={isSubmitting}>{isSubmitting ? 'Updating...' : 'Update Voucher'}</button>
                            </div>
                        </form>
                        <UpdatePopup 
