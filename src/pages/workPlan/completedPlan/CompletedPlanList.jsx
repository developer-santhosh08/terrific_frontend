...
import { TrashIcon, PencilSimpleIcon, ArrowBendUpLeftIcon } from '@phosphor-icons/react';
import TableSortIcon from '../../../components/TableSortIcon';
import { useSortableData } from '../../../hooks/useSortableData';
import SmartPagination from '../../../components/SmartPagination';
import { useLoader } from '../../../context/LoaderContext';
import DeletePopup from '../../../components/Popup/DeletePopup.jsx';
import RevertPopup from '../../../components/Popup/RevertPopup.jsx';
...
const CompletedPlanList = () => {
    ...
    const hasActionPermission = hasPermission('Work Plan.Completed Plan.Delete');
    ...
    useEffect(() => {
        const fetchClosedContacts = async () => {
            ...
            try {
                ...
                // Fetch the newly added closed sales contact API
                const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/workplan/work-plans/completed`, { headers });
                ...
                setContacts(data.map(item => {
                    ...
                    return {
                        id: item.id,
                        sno: `CW-${item.id.toString().padStart(2, '0')}`,
                        ...
                        currentStage: String(item.contact_status) === '2' ? 'Completed' : 'Completed'
                    };
                }));
            } catch (err) {
                console.error("Error fetching closed sales contacts:", err);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleConfirmRevert = async () => {
        ...
        try {
            const token = sessionStorage.getItem('erp_token');
            const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/workplan/work-plans/${pendingRevertId}/revert`, {
                method: 'PUT',
                headers: {
                    'Accept': 'application/json',
                    ...(token ? { 'Authorization': `Bearer ${token}` } : {})
            ...
            const json = await res.json();
            if (res.ok && (json.status === 'success' || json.message)) {
                setContacts(prev => prev.filter(c => c.id !== pendingRevertId));
            } else {
                alert(json.message || "Failed to revert");
            }
        } catch (err) {
            alert("Error reverting contact.");
        } finally {
    ...
    return (
        <section className="content">
            <style>{`
                ...
                .list-action-btn.btn-revert { background-color: #ef4444; color: #fff; }
                .list-action-btn.btn-revert:hover { background-color: #dc2626; }
            `}</style>

            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title max-[350px]:tw-text-center max-[350px]:tw-whitespace-nowrap max-[350px]:tw-text-lg">Completed Plan Details</h3>
                    </div>
                                ...
                                <table className="table table-bordered table-striped no-margin">
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="tw-align-middle tw-cursor-pointer hover:tw-bg-slate-50" onClick={() => requestSort('sno')}><div className="tw-flex tw-justify-between tw-items-center">Sno <TableSortIcon direction={getSortDirection('sno')} /></div></th>
                                                    ...
                                                    <td>
                                                        <div className="tw-flex tw-gap-2">
                                                            ...
                                                            <button 
                                                                ...
                                                                title="Edit"
                                                                onClick={() => navigate(`/work-plan/edit/${contact.id}`)}
                                                            >
                                                                <PencilSimpleIcon weight="duotone" className="tw-w-4 " />
                                                            </button>
                                                            */}
                                                            {hasPermission('Work Plan.Completed Plan.Delete') && (
                                                                <button 
                                                                    type="button" 
                                                            ...
                                                            {hasPermission('Work Plan.Work Plan.Edit') && (
                                                                <button 
                                                                    type="button" 
                                                                    className="list-action-btn btn-revert" 
                                                                    title="Revert to Infollowup"
                                                                    onClick={() => {
                                                                        setPendingRevertId(contact.id);
                        ...
                        <div className="tw-block md:tw-hidden tw-mt-4">
                            {paginatedContacts.map((contact) => (
                                <MobileCard key={contact.id}>
                                    <MobileCard.Header label="SNO" value={contact.sno} />
                                        ...
                                        {hasActionPermission && (
                                            <div className="tw-w-full tw-flex tw-justify-end tw-border-t tw-border-gray-100 tw-pt-2 tw-mt-1">
                                                ...
                                                {hasPermission('Work Plan.Work Plan.Edit') && (
                                                    <button 
                                                        type="button" 
                                                        className="list-action-btn btn-revert tw-shadow-none" 
                                                        title="Revert to Infollowup"
                                                        onClick={() => {
                                                            setPendingRevertId(contact.id);
                                                            setShowRevertPopup(true);
            ...
            <DeletePopup 
                ...
                message="Are you sure you want to delete this completed plan?"
            />
            ...
            <RevertPopup 
                ...
                message="Are you sure you want to <strong style='color: #dc2626'>revert</strong> this plan?<br />It will be moved back to the Work Plan details."
            />
        </section>
    );
};

export default CompletedPlanList;
