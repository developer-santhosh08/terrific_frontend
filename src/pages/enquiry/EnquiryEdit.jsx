...
const EnquiryEdit = () => {
    ...
    const [formData, setFormData] = useState({
        businessVertical: verticalOptions[0],
        engineerName: null,
        freightValue: 0,
        customerId: null,
        billingAddress1: '',
        ...
        const fetchAllData = async () => {
            ...
            try {
                ...
                const batch4 = await Promise.all([
                    ...
                    fetch(`${import.meta.env.VITE_API_BASE_URL}/api/enquiry/enquiry/${id}`),
                ...
                const [vendorMapRes, enquiryRes, pfRes, paymentRes] = batch4;
                ...
                let empOpts = [], custOpts = [], cityOpts = [], taxOptsList = [], prodOpts = [];
                let delOpts = [], warOpts = [], freightOpts = [], taxNoteOptsList = [];
                let pnfOptsList = [], paymentOptsList = [];

                if (empJson.status && empJson.data) empOpts = empJson.data.map(e => ({ value: e.id, label: e.name }));
                setEngineerOptions(empOpts);
                ...
                if (custJson.status && custJson.data) {
                    ...
                    custOpts = uniqueCustomers.map(c => ({ value: c.id, label: c.name }));
                    setCustomerOptions(custOpts);
                }

                if (cityJson.status && cityJson.data) {
                    cityOpts = cityJson.data.map(c => ({ value: c.id, label: c.name }));
                ...
                if (taxJson.status && taxJson.data) {
                    ...
                    setTaxOptions(taxOptsList);
                }

                if (prodJson.status && prodJson.data) {
                    prodOpts = prodJson.data.map(p => ({ value: p.id, label: p.name, price: p.price }));
                ...
                if (delJson) {
                    delOpts = [{ value: '', label: 'select' }, ...extractData(delJson).map(d => ({ value: d.id, label: d.name }))];
                    setDeliveryOptions(delOpts);
                }

                if (warJson) {
                    warOpts = [{ value: '', label: 'select' }, ...extractData(warJson).map(w => ({ value: w.id, label: w.name }))];
                    setWarrantyOptions(warOpts);
                }

                if (freightJson) {
                    freightOpts = [{ value: '', label: 'select' }, ...extractData(freightJson).map(f => ({ value: f.id, label: f.name }))];
                    setFreightOptions(freightOpts);
                }

                taxNoteOptsList = [{ value: '', label: 'select' }, ...extractData(taxNoteJson).map(t => ({ value: t.id, label: t.name }))];
                setTaxNoteOptions(taxNoteOptsList);

                pnfOptsList = [{ value: '', label: 'select' }, ...extractData(pfJson).map(p => ({ value: p.id, label: p.name }))];
                setPnfOptions(pnfOptsList);

                paymentOptsList = [{ value: '', label: 'select' }, ...extractData(paymentJson).map(p => ({ value: p.id, label: p.name }))];
                setPaymentOptions(paymentOptsList);

                const vendorMapList = extractData(vendorMapJson);
                setVendorMappings(vendorMapList);

                if (enquiryJson.status === 'success' && enquiryJson.data) {
                    const data = enquiryJson.data;

                    let c = {};
                    if (data.customer_id) {
                        try {
                            const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/enquiry/customers/${data.customer_id}`);
                            const json = await res.json();
                            if (json.status === 'success' && json.data) c = json.data;
                        } catch (err) { }
                    }

                    setFormData({
                        businessVertical: verticalOptions.find(opt => opt.value == data.business_vertical_id) || null,
                        engineerName: empOpts.find(e => e.value === data.allotted_id) || null,
                        customerId: custOpts.find(c => c.value === data.customer_id) || null,
                        city: cityOpts.find(c => c.value === data.city_id) || null,
                        billingAddress1: data.billing_address1 || '',
                        billingAddress2: data.billing_address2 || '',
                        billingAddress3: data.billing_address3 || '',
                        pincode: data.pincode || '',
                        committedDate: data.last_committed_date ? data.last_committed_date.split(' ')[0] : '',
                        referredBy: custOpts.find(c => c.value == data.referred_by) || null,
                        mobileNumber1: data.mobile_number1 || '',
                        mobileNumber2: data.mobile_number2 || '',
                        landlineNo: data.landline_number || '',
                        email: data.email || '',
                        website: data.website || '',
                        date: data.enquiry_date ? data.enquiry_date.split(' ')[0] : '',
                        freightValue: data.freight || 0,
                        contactPerson: data.cust_contact_name || c.cust_contact_name || '',
                        designation: data.cust_designation || c.cust_designation || '',
                        contactNumber: data.mobile_number2 || data.mobile_number1 || c.mobile_number2 || c.mobile_number1 || '',
                        convenientTime: data.time_from && data.time_from !== '00:00:00' ? (data.time_from + (data.time_to && data.time_to !== '00:00:00' ? ' to ' + data.time_to : '')) : (c.time_from && c.time_from !== '00:00:00' ? (c.time_from + (c.time_to && c.time_to !== '00:00:00' ? ' to ' + c.time_to : '')) : ''),
                        pnf: pnfOptsList.find(o => o.value == data.pf) || null,
                        taxNote: taxNoteOptsList.find(o => o.value == data.tax) || null,
                        payment: paymentOptsList.find(o => o.value == data.payment) || null,
                        delivery: delOpts.find(o => o.value == data.delivery) || null,
                        warranty: warOpts.find(o => o.value == data.warranty) || null,
                        frightDropdown: freightOpts.find(o => o.value == data.fright_id) || null,
                        remarks: data.customer_remark || '',
                        cashDiscount: data.cash_discount || 0
                    });

                    if (data.details && data.details.length > 0) {
                        const mappedProducts = data.details.map(d => ({
                            productId: d.product_id,
                            productName: d.product_name || (prodOpts.find(po => po.value === d.product_id)?.label) || '',
                            qty: Number(d.quantity) || 1,
                            rate: Number(d.original_rate) || 0,
                            discount: Number(d.discount) || 0,
                            buyback: Number(d.buyback) || 0,
                            pnf: Number(d.pf) || 0,
                            tax: Number(d.tax_percentage) || 0,
                            taxId: d.tax_id || null,
                            basic: Number(d.basic) || 0,
                            pnfAmount: Number(d.pf_amount) || 0,
                            taxAmount: Number(d.tax_amount) || 0,
                            amount: Number(d.total_value) || 0
                        }));
    ...
    const submitForm = async () => {
        ...
        try {
            ...
            // 1. Update Customer Profile (Master table)
            ...
            await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/enquiry/update-customer-profile/${id}`, {
                ...
                body: JSON.stringify({
                    ...
                    address3: formData.billingAddress3,
                    city_id: formData.city?.value || null,
                    contactRows: [{
            ...
            // 2. Update Enquiry
            const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/enquiry/enquiry/${id}`, {
                method: 'PUT',
                headers,
                body: JSON.stringify(payload)
            });
            ...
            if (json.status === 'success') {
                ...
                alert('Failed to update: ' + (json.message || 'Unknown error') + (json.error ? '\nDetails: ' + json.error : ''));
            }
        } catch (error) {
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border tw-flex tw-flex-col sm:tw-flex-row tw-justify-between sm:tw-items-center tw-gap-3">
                        <h3 className="card-title max-[768px]:tw-text-center max-[768px]:tw-w-full max-[350px]:tw-whitespace-nowrap max-[350px]:tw-text-lg">Edit Enquiry #{id}</h3>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="row">
                                ...
                                <div className="col-md-3 form-group" style={{ position: 'relative' }}>
                                    ...
                                    <Select options={customerOptions} value={formData.customerId} onChange={handleCustomerChange} placeholder="Choose a Customer" />
                            ...
                            <div className="row">
                                ...
                                <div className="col-md-3 form-group" style={{ position: 'relative' }}>
                                    ...
                                    <Select options={customerOptions} value={formData.referredBy} onChange={opt => handleChange('referredBy', opt)} placeholder="Referred By" />
                                </div>
                                <div className="col-md-3 form-group">
                                                            ...
                                                            <tr key={idx}>
                                                                <td style={{ minWidth: 350 }}>
                                                                    <Select
                                                                        ...
                                                                        value={
                                                                            productOptions.find(o => (row.productId && o.value == row.productId) || (row.productName && o.label === row.productName)) 
                                                                            || (row.productName ? { label: row.productName, value: row.productId || row.productName } : null)
                                                                        }
                                                                        onChange={opt => {
                            ...
                            <div className="form-actions mt-3 tw-flex max-[425px]:tw-flex-col tw-justify-between tw-gap-3">
                                ...
                                <button type="submit" className="btn-save max-[425px]:tw-w-full">Update Enquiry</button>
                            </div>
                        </form>

                        <CustomerPopup isOpen={showCustomerPopup} onClose={() => setShowCustomerPopup(false)} onSubmit={handleNewCustomerCreated} />
                        <ReferredCustomerPopup isOpen={showReferredPopup} onClose={() => setShowReferredPopup(false)} onSubmit={handleNewReferredCustomerCreated} />
                        <UpdatePopup 
