...
const CountyList = () => {
    ...
    const hasActionPermission = hasPermission('Power Master.Geo Locations - Country.Edit') || hasPermission('Power Master.Geo Locations - Country.Delete');
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await fetch(`${baseUrl}/api/master/country`, {
                    headers: {
                ...
                if (response.ok && json.status && Array.isArray(json.data)) {
                    setCountries([...json.data].reverse());
                }
            } catch (err) {
    ...
    const handleConfirmDelete = async () => {
        try {
            const token = sessionStorage.getItem('erp_token');
            const baseUrl = import.meta.env.VITE_API_BASE_URL || '';
            const response = await fetch(`${baseUrl}/api/master/country/${pendingDeleteId}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    ...(token ? { 'Authorization': `Bearer ${token}` } : {})
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title"> County</h3>
                        <div className="d-flex align-items-center tw-gap-4">
                            {hasPermission('Power Master.Geo Locations - Country.Add') && (
                                <button
                                    className="btn-create d-flex align-items-center tw-gap-1"
                                    onClick={() => navigate('/power-master/geolocations/country/add')}
                                >
                        ...
                        <div className="table-responsive tw-hidden md:tw-block">
                            <table className="table table-bordered table-striped no-margin">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th onClick={() => handleSort('name')} className="tw-cursor-pointer tw-select-none">Country Name <TableSortIcon direction={sortConfig.key === 'name' ? sortConfig.direction : null} /></th>
                                        ...
                                        <tr key={index}>
                                            ...
                                            <td className="align-middle">
                                                <span className={`badge ${e.status === 'Active' || e.status === 1 || e.status_label === 'Active' ? 'bg-success' : 'bg-danger'}`}>
                                                    {e.status_label || (e.status === 1 ? 'Active' : e.status)}
                                                </span>
                                            </td>
                                            {hasActionPermission && (
<td className="align-middle">
                                                <div className="tw-flex tw-gap-2">
                                                    {hasPermission('Power Master.Geo Locations - Country.Edit') && (
<button type="button" className="list-action-btn btn-edit" onClick={() => navigate(`/power-master/geolocations/country/edit/${e.id}`)}>
                                                        <PencilSimpleIcon weight="duotone" className="tw-w-4" />
                                                    </button>
)}
                                                    {hasPermission('Power Master.Geo Locations - Country.Delete') && (
<button
                                                        type="button"
                                ...
                                <MobileCard key={index}>
                                    ...
                                    <MobileCard.Body>
                                        <div className="tw-grid tw-grid-cols-2 tw-gap-3">
                                            <MobileCard.Field label="Country Name" value={e.name} />
