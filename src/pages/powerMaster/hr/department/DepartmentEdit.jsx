...
const DepartmentEdit = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setFetchError('');
            try {
                const result = await apiFetch(`/master/department/${itemId}`);
                if (!result) return;
                const { res, json } = result;
                if (json.status) {
                    const d = json.data;
                    setName(d.name || '');
                    setCumTarget(cumTargetOptions.find(o => o.value === d.cum_target) ?? cumTargetOptions[0]);
                    setStatus(statusOptions.find(o => o.value === d.status) ?? statusOptions[0]);
                } else {
                    setFetchError(json.message || 'Failed to fetch department.');
                }
            } catch {
        ...
        try {
            ...
            if (json.status) {
                ...
                const detail = json.errors
                    ...
                    : json.message;
                setApiError(detail || 'Failed to update department.');
            }
        } catch {
