...
const VendorGroupList = () => {
    ...
    const hasActionPermission = hasPermission('Power Master.Vendor - Vendor Group.Edit') || hasPermission('Power Master.Vendor - Vendor Group.Delete');
...
const json = res?.json || {};
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            const res = await apiFetch(`/master/vendorGroup/${pendingDeleteId}`, { 
                method: 'DELETE' 
            });
const data = res?.json || {};
            if (data.status) {
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title">Vendor Group Details</h3>
                        <div className="d-flex align-items-center tw-gap-4">
                            {hasPermission('Power Master.Vendor - Vendor Group.Add') && (
                                <button
                                    className="btn-create d-flex align-items-center tw-gap-1"
                                    onClick={() => navigate('/power-master/vendor/vendor-group/add')}
                                >
                        ...
                        <div className="table-responsive">
                            <table className="table table-bordered table-striped no-margin">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th onClick={() => handleSort('vendorGroup')} className="tw-cursor-pointer tw-select-none">Vendor Group <TableSortIcon direction={sortConfig.key === 'vendorGroup' ? sortConfig.direction : null} /></th>
                                ...
                                <tbody>
                                    {sortedData.map((e, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="align-middle">{e.vendorGroup}</td>
...
<td className="align-middle">
                                                <div className="tw-flex tw-gap-2">
                                                    {hasPermission('Power Master.Vendor - Vendor Group.Edit') && (
<button type="button" className="list-action-btn btn-edit" onClick={() => navigate(`/power-master/vendor/vendor-group/edit/${e.id}`)}>
                                                        <PencilSimpleIcon weight="duotone" className="tw-w-4" />
                                                    </button>
)}
                                                    {hasPermission('Power Master.Vendor - Vendor Group.Delete') && (
<button
                                                        type="button"
