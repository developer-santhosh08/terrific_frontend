...
const ContraPersonList = () => {
    const { hasPermission } = usePermissions();
    const hasActionPermission = hasPermission('Power Master.Contra - Contra Person.Edit') || hasPermission('Power Master.Contra - Contra Person.Delete');
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError('');
            try {
                const result = await apiFetch(`/master/contra-master`, { cache: 'no-store' });
                if (!result) return;
                ...
                if (json.status) {
                    ...
                    setError(json.message || 'Failed to fetch contra persons.');
                }
            } catch {
    ...
    const handleConfirmDelete = async () => {
        try {
            await apiFetch(`/master/contra-master/${pendingDeleteId}`, {
                method: 'DELETE'
            });
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title"> Contra Person</h3>
                        <div className="d-flex align-items-center tw-gap-4">
                            {hasPermission('Power Master.Contra - Contra Person.Add') && (
                                <button
                                    className="btn-create d-flex align-items-center tw-gap-1"
                                    onClick={() => navigate('/power-master/contra/contra-person/add')}
                                >
                            ...
                            <table className="table table-bordered table-striped no-margin">
                                <thead>
                                    <tr>
                                        ...
                                        <th onClick={() => handleSort('mobile_number')} className="tw-cursor-pointer tw-select-none">Phone Number <TableSortIcon direction={sortConfig.key === 'mobile_number' ? sortConfig.direction : null} /></th>
                                ...
                                <tbody>
                                    {sortedData.map((e, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="align-middle">{e.mobile_number}</td>
                                            <td className="align-middle">
                                                <span className={`badge ${e.status == 1 || e.status === 'Active' ? 'bg-success' : 'bg-danger'}`}>
                                                    {e.status == 1 || e.status === 'Active' ? 'Active' : 'Inactive'}
                                                </span>
                                            </td>
                                            {hasActionPermission && (
<td className="align-middle">
                                                <div className="tw-flex tw-gap-2">
                                                    {hasPermission('Power Master.Contra - Contra Person.Edit') && (
<button type="button" className="list-action-btn btn-edit" onClick={() => navigate(`/power-master/contra/contra-person/edit/${e.id}`)}>
                                                        <PencilSimpleIcon weight="duotone" className="tw-w-4" />
                                                    </button>
)}
                                                    {hasPermission('Power Master.Contra - Contra Person.Delete') && (
<button
                                                        type="button"
                                ...
                                <MobileCard key={index}>
                                    ...
                                    <MobileCard.Body>
                                        <div className="tw-grid tw-grid-cols-2 tw-gap-3">
                                            ...
                                            <MobileCard.Field label="Phone Number" value={e.mobile_number} />
...
export default ContraPersonList;
