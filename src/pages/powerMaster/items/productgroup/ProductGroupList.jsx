...
const ProductGroupList = () => {
    ...
    const hasActionPermission = hasPermission('Power Master.Items - Product Group.Edit') || hasPermission('Power Master.Items - Product Group.Delete');
    ...
    const handleConfirmDelete = async () => {
        try {
            const response = await apiFetch(`/master/productGroup/${pendingDeleteId}`, {
                method: 'DELETE'
            });
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title">Product Group Details</h3>
                        <div className="d-flex align-items-center tw-gap-4">
                            {hasPermission('Power Master.Items - Product Group.Add') && (
                                <button
                                    className="btn-create d-flex align-items-center tw-gap-1"
                                    onClick={() => navigate('/power-master/items/product-group/add')}
                                >
                        ...
                        <div className="table-responsive tw-hidden md:tw-block">
                            <table className="table table-bordered table-striped no-margin">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th onClick={() => handleSort('productGroup')} className="tw-cursor-pointer tw-select-none">Product Group <TableSortIcon direction={sortConfig.key === 'productGroup' ? sortConfig.direction : null} /></th>
                                    ...
                                    {loading ? (
                                        ...
                                        sortedData.map((e, index) => (
                                            <tr key={e.id}>
                                                ...
                                                <td className="align-middle">{e.productGroup}</td>
...
<td className="align-middle">
                                                    <div className="tw-flex tw-gap-2">
                                                        {hasPermission('Power Master.Items - Product Group.Edit') && (
<button type="button" className="list-action-btn btn-edit" onClick={() => navigate(`/power-master/items/product-group/edit/${e.id}`)}>
                                                            <PencilSimpleIcon weight="duotone" className="tw-w-4" />
                                                        </button>
)}
                                                        {hasPermission('Power Master.Items - Product Group.Delete') && (
<button
                                                            type="button"
                                    ...
                                    <MobileCard key={e.id}>
                                        ...
                                        <MobileCard.Body>
                                            <div className="tw-grid tw-grid-cols-2 tw-gap-3">
                                                <MobileCard.Field label="Product Group" value={e.productGroup} />
