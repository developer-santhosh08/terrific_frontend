...
const PaymentNotesList = () => {
    ...
    const hasActionPermission = hasPermission('Power Master.Enquiry Master - Payment Notes.Edit') || hasPermission('Power Master.Enquiry Master - Payment Notes.Delete');
    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/master/payment-note`, {
                    cache: 'no-store',
                    headers: {
    ...
    const handleConfirmDelete = async () => {
        try {
            const token = sessionStorage.getItem('erp_token');
            await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/master/payment-note/${pendingDeleteId}`, {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
                    ...(token ? { 'Authorization': `Bearer ${token}` } : {})
                ...
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        ...
                        <div className="d-flex align-items-center tw-gap-4">
                            {hasPermission('Power Master.Enquiry Master - Payment Notes.Add') && (
                                <button
                                    className="btn-create d-flex align-items-center tw-gap-1"
                                    onClick={() => navigate('/power-master/payment-notes/add')}
                                >
...
<td className="align-middle">
                                                <div className="tw-flex tw-gap-2">
                                                    {hasPermission('Power Master.Enquiry Master - Payment Notes.Edit') && (
<button type="button" className="list-action-btn btn-edit" onClick={() => navigate(`/power-master/payment-notes/edit/${e.id}`)}>
                                                        <PencilSimpleIcon weight="duotone" className="tw-w-4" />
                                                    </button>
)}
                                                    {hasPermission('Power Master.Enquiry Master - Payment Notes.Delete') && (
<button
                                                        type="button"
...
export default PaymentNotesList;
