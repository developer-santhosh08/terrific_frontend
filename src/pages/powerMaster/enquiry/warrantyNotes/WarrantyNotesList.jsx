...
const WarrantyNotesList = () => {
    ...
    const hasActionPermission = hasPermission('Power Master.Enquiry Master - Warranty Notes.Edit') || hasPermission('Power Master.Enquiry Master - Warranty Notes.Delete');
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/master/warranty-notes`, {
                    cache: 'no-store',
                    headers: {
                ...
                if (data?.status && data?.data) {
                    ...
                    setError(data?.message || 'Failed to fetch warranty notes.');
                }
            } catch (error) {
                setError('Network error. Please try again.');
                console.error("Error fetching warranty notes:", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleConfirmDelete = async () => {
        try {
            const token = sessionStorage.getItem('erp_token');
            await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/master/warranty-notes/${pendingDeleteId}`, {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
                    ...(token ? { 'Authorization': `Bearer ${token}` } : {})
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title"> Warranty Notes</h3>
                        <div className="d-flex align-items-center tw-gap-4">
                            {hasPermission('Power Master.Enquiry Master - Warranty Notes.Add') && (
                                <button
                                    className="btn-create d-flex align-items-center tw-gap-1"
                                    onClick={() => navigate('/power-master/warranty-notes/add')}
                                >
...
<td className="align-middle">
                                                <div className="tw-flex tw-gap-2">
                                                    {hasPermission('Power Master.Enquiry Master - Warranty Notes.Edit') && (
<button type="button" className="list-action-btn btn-edit" onClick={() => navigate(`/power-master/warranty-notes/edit/${e.id}`)}>
                                                        <PencilSimpleIcon weight="duotone" className="tw-w-4" />
                                                    </button>
)}
                                                    {hasPermission('Power Master.Enquiry Master - Warranty Notes.Delete') && (
<button
                                                        type="button"
