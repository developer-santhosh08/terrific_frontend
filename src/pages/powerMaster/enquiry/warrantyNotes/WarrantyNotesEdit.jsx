...
const WarrantyNotesEdit = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/master/warranty-notes/${itemId}`, {
                    headers: {
                ...
                if (data?.status && data?.data) {
                    ...
                    setFetchError(data?.message || 'Failed to fetch warranty note.');
                }
            } catch (error) {
                console.error("Error fetching warranty note details:", error);
    ...
    const handleConfirmUpdate = async () => {
        ...
        try {
            ...
            const data = await response.json();
            if (data?.status) {
                navigate('/power-master/warranty-notes');
            } else {
                setApiError(data?.message || 'Failed to update warranty note.');
            }
        } catch {
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title">Edit Warranty Notes</h3>
                        <button className="btn-header-back" onClick={() => navigate('/power-master/warranty-notes')}>
                            Back
                        </button>
                    </div>
                        ...
                        {loading ? (
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="form-actions mt-3 d-flex justify-content-between gap-2 flex-wrap">
                                    <button type="button" className="btn-cancel" onClick={() => navigate('/power-master/warranty-notes')}>Cancel</button>
