...
const EnquirySoruceList = () => {
    const { hasPermission } = usePermissions();
    const hasActionPermission = hasPermission('Power Master.Enquiry Master - Enquiry Source.Edit') || hasPermission('Power Master.Enquiry Master - Enquiry Source.Delete');
    ...
    const fetchEnquirySources = async () => {
        try {
            ...
            if (data?.status && data?.data) {
                setEnquirySources(data.data.reverse());
            }
        } catch (error) {
    ...
    const handleConfirmDelete = async () => {
        try {
            const token = sessionStorage.getItem('erp_token');
            await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/master/enquiry-source/${pendingDeleteId}`, { 
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
                    ...(token ? { 'Authorization': `Bearer ${token}` } : {})
            ...
            setEnquirySources(prev => prev.filter(d => d.id !== pendingDeleteId));
        } catch {
            // silently ignore
        } finally {
    ...
    } = useTableControls(enquirySources);
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title">Enquiry Source Details</h3>
                        {hasPermission('Power Master.Enquiry Master - Enquiry Source.Add') && (
                            <button
                                className="btn-create"
                                onClick={() => navigate('/power-master/enquiry-source/add')}
                            >
                        ...
                        <div className="table-responsive tw-hidden md:tw-block">
                            <table className="table table-bordered table-striped no-margin">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th onClick={() => handleSort('name')} className="tw-cursor-pointer tw-select-none">Enquiry Source <TableSortIcon direction={sortConfig.key === 'name' ? sortConfig.direction : null} /></th>
                                ...
                                <tbody>
                                    {sortedData.map((s, index) => (
                                        <tr key={s.id}>
                                            ...
                                            <td>{s.name}</td>
                                            <td>
                                                <span className="badge badge-success" style={{ backgroundColor: s.status === 1 ? '#28a745' : '#dc3545' }}>
                                                    {s.status_label || (s.status === 1 ? 'Active' : 'Inactive')}
                                                </span>
                                            </td>
                                            {hasActionPermission && (
<td>
                                                <div className="tw-flex tw-gap-2">
                                                    {hasPermission('Power Master.Enquiry Master - Enquiry Source.Edit') && (
<button type="button" className="list-action-btn btn-edit" onClick={() => navigate(`/power-master/enquiry-source/edit/${s.id}`)}>
                                                        <PencilSimpleIcon weight="duotone" className="tw-w-4" />
                                                    </button>
)}
                                                    {hasPermission('Power Master.Enquiry Master - Enquiry Source.Delete') && (
<button
                                                        type="button"
                                ...
                                <MobileCard key={s.id}>
                                    ...
                                    <MobileCard.Body>
                                        <div className="tw-grid tw-grid-cols-2 tw-gap-3">
                                            <MobileCard.Field label="Enquiry Source" value={s.name} />
...
export default EnquirySoruceList;
