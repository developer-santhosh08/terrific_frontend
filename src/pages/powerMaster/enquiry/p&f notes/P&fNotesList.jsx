...
const PandFNotesList = () => {
    const { hasPermission } = usePermissions();
    const hasActionPermission = hasPermission('Power Master.Enquiry Master - P&F Notes.Edit') || hasPermission('Power Master.Enquiry Master - P&F Notes.Delete');
    ...
    useEffect(() => {
        const fetchNotes = async () => {
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/master/pf-note`, {
                    cache: 'no-store',
                    headers: {
                ...
                console.error("Error fetching pf notes:", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleConfirmDelete = async () => {
        try {
            const token = sessionStorage.getItem('erp_token');
            await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/master/pf-note/${pendingDeleteId}`, {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
                    ...(token ? { 'Authorization': `Bearer ${token}` } : {})
            ...
            setNotes(prev => prev.filter(d => d.id !== pendingDeleteId));
        } catch (error) {
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title">P&F Notes</h3>
                        {hasPermission('Power Master.Enquiry Master - P&F Notes.Add') && (
                            <button
                                className="btn-create"
                                onClick={() => navigate('/power-master/pf-notes/add')}
                            >
                                ...
                                <tbody>
                                    {sortedData.map((n, index) => (
                                        <tr key={n.id}>
                                            ...
                                            <td>{n.name}</td>
                                            <td>
                                                <span className="badge badge-success" style={{ backgroundColor: n.status === 1 ? '#28a745' : '#dc3545' }}>
                                                    {n.status_label || (n.status === 1 ? 'Active' : 'Inactive')}
                                                </span>
                                            </td>
                                            {hasActionPermission && (
<td>
                                                <div className="tw-flex tw-gap-2">
                                                    {hasPermission('Power Master.Enquiry Master - P&F Notes.Edit') && (
<button type="button" className="list-action-btn btn-edit" onClick={() => navigate(`/power-master/pf-notes/edit/${n.id}`)}>
                                                        <PencilSimpleIcon weight="duotone" className="tw-w-4" />
                                                    </button>
)}
                                                    {hasPermission('Power Master.Enquiry Master - P&F Notes.Delete') && (
<button
                                                        type="button"
                                ...
                                <MobileCard key={n.id}>
                                    ...
                                    <MobileCard.Body>
                                        <div className="tw-grid tw-grid-cols-2 tw-gap-3">
                                            <MobileCard.Field label="Name" value={n.name} />
