...
const UserGroupList = () => {
    ...
    const [data, setData] = useState([
        { id: 1, userGroupName: 'SERVICE ENGINEER', status: 'Active' },
        { id: 2, userGroupName: 'SALES ENGINEER', status: 'Active' }
    ]);
    ...
    const handleConfirmDelete = async () => {
        try {
            await apiFetch(`/master/user-group/${pendingDeleteId}`, { method: 'DELETE' });
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title">User Group Details</h3>
                        <div className="d-flex align-items-center tw-gap-4">
                            <button
                                className="btn-create d-flex align-items-center tw-gap-1"
                                onClick={() => navigate('/power-master/user/user-group/add')}
                            >
                        ...
                        <div className="table-responsive">
                            <table className="table table-bordered table-striped no-margin">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th onClick={() => handleSort('userGroupName')} className="tw-cursor-pointer tw-select-none">User Group Name <TableSortIcon direction={sortConfig.key === 'userGroupName' ? sortConfig.direction : null} /></th>
                                ...
                                <tbody>
                                    {sortedData.map((e, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="align-middle">{e.userGroupName}</td>
...
<button type="button" className="list-action-btn btn-edit" onClick={() => navigate(`/power-master/user/user-group/edit/${e.id}`)}>
