...
const AccountsCategoryList = () => {
    ...
    const hasActionPermission = hasPermission('Power Master.Bank - Accounts Category.Edit') || hasPermission('Power Master.Bank - Accounts Category.Delete');
        ...
        try {
            ...
            if (json.status && json.data) {
                const formattedData = json.data.map(item => ({
                    id: item.id,
                    accountsCategory: item.name,
                    status: (item.status === 1 || item.status === '1' || item.status === 'Active') ? 'Active' : 'Inactive'
                })).sort((a, b) => b.id - a.id);
                setData(formattedData);
            }
        } catch (err) {
            console.error("Error fetching accounts category:", err);
        }
    };
    ...
    const handleConfirmDelete = async () => {
        try {
            const token = sessionStorage.getItem('erp_token');
            await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/master/accounts-category/${pendingDeleteId}`, {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title"> Accounts Category</h3>
                        <div className="d-flex align-items-center tw-gap-4">
                            {hasPermission('Power Master.Bank - Accounts Category.Add') && (
                                <button
                                    className="btn-create d-flex align-items-center tw-gap-1"
                                    onClick={() => navigate('/power-master/bank/accounts-category/add')}
                                >
                        ...
                        <div className="table-responsive tw-hidden md:tw-block">
                            <table className="table table-bordered table-striped no-margin">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th onClick={() => handleSort('accountsCategory')} className="tw-cursor-pointer tw-select-none">Accounts Category <TableSortIcon direction={sortConfig.key === 'accountsCategory' ? sortConfig.direction : null} /></th>
                                ...
                                <tbody>
                                    {sortedData.map((e, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="align-middle">{e.accountsCategory}</td>
...
<td className="align-middle">
                                                <div className="tw-flex tw-gap-2">
                                                    {hasPermission('Power Master.Bank - Accounts Category.Edit') && (
<button type="button" className="list-action-btn btn-edit" onClick={() => navigate(`/power-master/bank/accounts-category/edit/${e.id}`)}>
                                                        <PencilSimpleIcon weight="duotone" className="tw-w-4" />
                                                    </button>
)}
                                                    {hasPermission('Power Master.Bank - Accounts Category.Delete') && (
<button
                                                        type="button"
                                ...
                                <MobileCard key={index}>
                                    ...
                                    <MobileCard.Body>
                                        <div className="tw-grid tw-grid-cols-2 tw-gap-3">
                                            <MobileCard.Field label="Accounts Category" value={e.accountsCategory} />
...
export default AccountsCategoryList;
