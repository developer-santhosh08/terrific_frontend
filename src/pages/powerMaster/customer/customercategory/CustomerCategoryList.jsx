...
const CustomerCategoryList = () => {
    ...
    const hasActionPermission = hasPermission('Power Master.Customer - Customer Category.Edit') || hasPermission('Power Master.Customer - Customer Category.Delete');
    ...
    const handleConfirmDelete = async () => {
        try {
            await fetch(import.meta.env.VITE_API_BASE_URL + `/api${'/master/customer-category/${pendingDeleteId}'}`, {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title">Customer Category Details</h3>
                        <div className="d-flex align-items-center tw-gap-4">
                            {hasPermission('Power Master.Customer - Customer Category.Add') && (
                                <button
                                    className="btn-create d-flex align-items-center tw-gap-1"
                                    onClick={() => navigate('/power-master/customer/customer-category/add')}
                                >
                        ...
                        <div className="table-responsive tw-hidden md:tw-block">
                            <table className="table table-bordered table-striped no-margin">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th onClick={() => handleSort('customerCategory')} className="tw-cursor-pointer tw-select-none">Customer Category <TableSortIcon direction={sortConfig.key === 'customerCategory' ? sortConfig.direction : null} /></th>
                                ...
                                <tbody>
                                    {sortedData.map((e, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="align-middle">{e.customerCategory}</td>
                                            ...
                                            {hasActionPermission && (
                                                <td className="align-middle">
                                                    <div className="tw-flex tw-gap-2">
                                                        {hasPermission('Power Master.Customer - Customer Category.Edit') && (
                                                            <button type="button" className="list-action-btn btn-edit" onClick={() => navigate(`/power-master/customer/customer-category/edit/${e.id}`)}>
                                                                <PencilSimpleIcon weight="duotone" className="tw-w-4" />
                                                            </button>
                                                        )}
                                                        {hasPermission('Power Master.Customer - Customer Category.Delete') && (
                                                            <button
                                                                type="button"
                                ...
                                <MobileCard key={index}>
                                    ...
                                    <MobileCard.Body>
                                        <div className="tw-grid tw-grid-cols-2 tw-gap-3">
                                            <MobileCard.Field label="Customer Category" value={e.customerCategory} />
