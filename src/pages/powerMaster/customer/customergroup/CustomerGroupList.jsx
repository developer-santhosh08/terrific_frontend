...
const CustomerGroupList = () => {
    ...
    const hasActionPermission = hasPermission('Power Master.Customer - Customer Group.Edit') || hasPermission('Power Master.Customer - Customer Group.Delete');
    ...
    const handleConfirmDelete = async () => {
        try {
            await fetch(import.meta.env.VITE_API_BASE_URL + `/api${'/master/customer-group/${pendingDeleteId}'}`, {
                method: 'DELETE',
                headers: {
                    'Accept': 'application/json',
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border d-flex justify-content-between align-items-center">
                        <h3 className="card-title">Customer Group Details</h3>
                        <div className="d-flex align-items-center tw-gap-4">
                            {hasPermission('Power Master.Customer - Customer Group.Add') && (
                                <button
                                    className="btn-create d-flex align-items-center tw-gap-1"
                                    onClick={() => navigate('/power-master/customer/customer-group/add')}
                                >
                        ...
                        <div className="table-responsive tw-hidden md:tw-block">
                            <table className="table table-bordered table-striped no-margin">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th onClick={() => handleSort('customerGroup')} className="tw-cursor-pointer tw-select-none">Customer Group <TableSortIcon direction={sortConfig.key === 'customerGroup' ? sortConfig.direction : null} /></th>
                                ...
                                <tbody>
                                    {sortedData.map((e, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="align-middle">{e.customerGroup}</td>
...
<td className="align-middle">
                                                <div className="tw-flex tw-gap-2">
                                                    {hasPermission('Power Master.Customer - Customer Group.Edit') && (
<button type="button" className="list-action-btn btn-edit" onClick={() => navigate(`/power-master/customer/customer-group/edit/${e.id}`)}>
                                                        <PencilSimpleIcon weight="duotone" className="tw-w-4" />
                                                    </button>
)}
                                                    {hasPermission('Power Master.Customer - Customer Group.Delete') && (
<button
                                                        type="button"
                                ...
                                <MobileCard key={index}>
                                    ...
                                    <MobileCard.Body>
                                        <div className="tw-grid tw-grid-cols-2 tw-gap-3">
                                            <MobileCard.Field label="Customer Group" value={e.customerGroup} />
