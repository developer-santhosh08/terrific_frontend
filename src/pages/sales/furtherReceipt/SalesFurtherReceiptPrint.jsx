...
const styles = StyleSheet.create({
    ...
    colModel: { width: '13%', borderRightWidth: 1, borderRightColor: '#000', padding: 5, textAlign: 'center', justifyContent: 'center' },
    colCap: { width: '9%', borderRightWidth: 1, borderRightColor: '#000', padding: 5, textAlign: 'center', justifyContent: 'center' },
    colDesc: { width: '23%', borderRightWidth: 1, borderRightColor: '#000', padding: 5, justifyContent: 'center' },
...
const PODocument = ({ data, isAdvance }) => {
    const { header, details, company, tax_breakdown, special_instructions, payment_history, balance_total, total_paid_amount, bank_details } = data;

    return (
        <Document title={header?.customer_name ? `Receipt-${header?.po_no ? `${header.po_no} ` : ''}${header.customer_name}` : 'Advanced Payment Receipt'}>
            <Page size="A4" style={styles.page}>
                <View style={styles.pageBorder}>

                    <Text style={styles.title}>{isAdvance ? 'ADVANCED RECEIPT' : 'FURTHER RECEIPT'}</Text>

                    <View style={styles.companyHeader}>
                        <View style={styles.companyInfo}>
                            <Text style={styles.companyName}>{company?.name || 'Terrific Technologies'}</Text>
                            <Text style={styles.companyAddress}>{company?.address1 || '62/1, Chitralaya Nagar, Nallur, Tamil Nadu, Tiruppur-641606'}</Text>
                            <Text style={styles.companyAddress}>Mobile : {company?.mobile || '9944680677'}</Text>
                            <Text style={[styles.companyAddress, { fontFamily: 'Helvetica-Bold' }]}>GSTIN NO: {company?.tin || '33AAYFT1975H1ZK'}</Text>
                        </View>
                    ...
                    <View style={styles.row}>
                        <View style={{ width: '60%', borderRightWidth: 1, borderRightColor: '#000', padding: 5 }}>
                            <Text style={styles.bold}>Invoice To</Text>
                            <Text style={styles.bold}>{header?.customer_name}</Text>
                            <Text>{header?.customer_address1}</Text>
                            <Text>{header?.customer_address2}</Text>
                            <Text>{header?.customer_address3}</Text>
                            <Text>Mobile: {header?.customer_mobile}, Phone: {header?.customer_phone}</Text>
                            <Text>Contact Person : {header?.contact_person_name}</Text>
                            <Text>Contact No : {header?.customer_contact_no}</Text>
                        </View>
                        <View style={{ width: '40%', padding: 5, justifyContent: 'center' }}>
                        ...
                        <View style={{ width: '65%', ...styles.col, borderBottomWidth: 0, padding: 10 }}>
                            ...
                            {bank_details && (
                                <View style={{ marginTop: 10 }}>
                                    <Text style={{ ...styles.bold, textDecoration: 'underline', marginBottom: 5 }}>Bank Details</Text>
                                    <View style={styles.flexRowBetween}><Text style={{ width: '40%', ...styles.bold }}>Name:</Text><Text style={{ width: '60%' }}>{bank_details.name || ''}</Text></View>
                                    <View style={styles.flexRowBetween}><Text style={{ width: '40%', ...styles.bold }}>A/c No:</Text><Text style={{ width: '60%' }}>{bank_details.ac_no || ''}</Text></View>
                                    <View style={styles.flexRowBetween}><Text style={{ width: '40%', ...styles.bold }}>Branch:</Text><Text style={{ width: '60%' }}>{bank_details.branch || ''}</Text></View>
                                    <View style={styles.flexRowBetween}><Text style={{ width: '40%', ...styles.bold }}>IFSC Code:</Text><Text style={{ width: '60%' }}>{bank_details.ifsc || ''}</Text></View>
                                    <View style={styles.flexRowBetween}><Text style={{ width: '40%', ...styles.bold }}>Bank:</Text><Text style={{ width: '60%' }}>{bank_details.bank || ''}</Text></View>
                                    <View style={styles.flexRowBetween}><Text style={{ width: '40%', ...styles.bold }}>Account Type:</Text><Text style={{ width: '60%' }}>{bank_details.account_type || ''}</Text></View>
                                </View>
                            )}
                        </View>
...
const SalesFurtherReceiptPrint = () => {
    const { id } = useParams();
    const isAdvance = window.location.href.includes('type=advance') || (document.referrer && document.referrer.includes('advanced-receipt'));
    ...
    useEffect(() => {
        const fetchPrintData = async () => {
            ...
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/sales/further-payments/receipts-print/${id}`, { headers });
    ...
    const fileName = printData.header?.customer_name ? `Receipt-${printData.header?.po_no ? `${printData.header.po_no} ` : ''}${printData.header.customer_name}.pdf` : 'Further_Receipt.pdf';
    document.title = fileName;

    const handleDownload = async () => {
        setLoading(true);
        try {
            const blob = await pdf(<PODocument data={printData} isAdvance={isAdvance} />).toBlob();
        ...
        <div style={{ display: 'flex', flexDirection: 'column', width: '100vw', height: '100vh', margin: 0, padding: 0 }}>
            ...
            <div style={{ flex: 1 }}>
                <PDFViewer width="100%" height="100%" style={{ border: 'none' }} title={fileName}>
                    <PODocument data={printData} isAdvance={isAdvance} />
                </PDFViewer>
            </div>
        </div>
    );
...
export default SalesFurtherReceiptPrint;
