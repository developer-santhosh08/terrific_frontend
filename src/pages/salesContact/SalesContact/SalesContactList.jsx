...
const SalesContactList = () => {
    ...
    const fetchContacts = async () => {
        ...
        try {
            ...
            const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/salescontact/sales-contact`, { headers });
            ...
            let mapped = data.map(item => {
                ...
                return {
                    ...
                    currentStage: String(item.contact_status) === '1' ? 'In Followup' :
                        String(item.contact_status) === '2' ? 'Closed' :
                            String(item.contact_status) === '3' ? 'Converted' :
                                String(item.contact_status) === '4' ? 'Hold' : 'In Followup'
                };
            });
    ...
    const handleDateChange = async (id, newDate, remark) => {
        ...
        try {
            const token = sessionStorage.getItem('erp_token');
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/salescontact/sales-contact/update-followup/${id}`, {
                method: 'PUT',
                headers: {
                    'Accept': 'application/json',
            ...
            <div className="container-fluid">
                <div className="card">
                    <div className="card-header with-border tw-flex tw-flex-col sm:tw-flex-row tw-justify-between sm:tw-items-center tw-gap-3">
                        <h3 className="card-title tw-mb-0 max-[350px]:tw-text-center max-[350px]:tw-whitespace-nowrap max-[350px]:tw-text-lg">Sales Contact Details</h3>
                        {hasPermission('Sales Contact.Sales Contact.Add') && (
                            <button className="btn-create tw-w-full sm:tw-w-auto" onClick={() => navigate('/sales-contact/add')}>
                                Add Sales Contact
                            </button>
                        )}
                    </div>
                                                            ...
                                                            <div className="tw-flex tw-gap-2">
                                                                {hasPermission('Sales Contact.Sales Contact.Edit') && (
                                                                    <button
                                                                        ...
                                                                        title="Edit"
                                                                        onClick={() => navigate(`/sales-contact/edit/${contact.id}`, { state: { from: '/sales-contact' } })}
                                                                    >
                                                                        <PencilSimpleIcon weight="duotone" className="tw-w-4 " />
                                                                    </button>
                                                                )}
                                                                {hasPermission('Sales Contact.Sales Contact.Delete') && (
                                                                    <button
                                                                        type="button"
